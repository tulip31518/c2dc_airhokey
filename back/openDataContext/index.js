...
function drawRankPanel() {
  ...
  context.textAlign = 'center';//console.log("fontsize", fontSize+10, RankWidth/2.5);
  ...
  if(friendData == undefined) return;
 ...
 * 根据绘制信息以及当前i绘制元素
 */
function getNoNChineseCharacterCount(str) {
...
function drawByData(data, i) {
  ...
  let nonChinaCount = getNoNChineseCharacterCount(tempname);  
  let maxNameWidth = 0;
  maxNameWidth = (tempname.length - nonChinaCount) * (RankWidth / 12) + nonChinaCount * (RankWidth / 15);
  context.fillText(data.name + "", x, startY + i * preOffsetY + textOffsetY,  maxNameWidth);
  x += textMaxSize + intervalX;

  //绘制分数 score
  context.textAlign = 'left';
  var temp = RankWidth / 7;
  let tempscore = "";
  tempscore = data.score;
  let maxScoreWidth = 0;
  maxScoreWidth = (RankWidth * tempscore.length) / 24; 
  context.font = (fontSize - 5) + "px Arial";
  
  context.fillText(data.score + "", RankWidth - intervalX, startY + i * preOffsetY + textOffsetY, maxScoreWidth);
...
function dealAndSortData(source) {
  ...
  source.forEach(element => {
	...
	if(element['KVDataList'].length == 0) {
		item['score'] = 0;
	}else {
